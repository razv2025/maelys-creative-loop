...
import { BRAND } from "@/lib/maelys";

// Renders generated LP content in an authentic MAËLYS skin (tokens extracted
// from their live LP CSS). Images are placeholder blocks — in production the
// asset layer pulls from the brand DAM / wown CDN.
...
function Jar({ size = 120 }: { size?: number }) {
  // Stylized stand-in for the purple GET-DREAMY jar product shot.
  return (
    <div
    ...
    >
      <div
        className="absolute inset-x-[12%] top-0 h-[22%] rounded-t-xl"
        style={{ background: "#c9c5f2" }}
      />
      <div
...
export default function LandingPagePreview({
  ...
  return (
    ...
    >
      ...
      {/* Header */}
      <div className="flex items-center justify-center border-b border-black/5 py-3">
        {logoOk ? (
      ...
      {/* Hero */}
      <section className="px-6 pb-10 pt-8 text-center" style={{ background: "var(--m-pale-pink)" }}>
        {c.hero.eyebrow && (
      ...
      {/* Trust ticker */}
      <div className="overflow-hidden border-y border-black/5 bg-white py-2">
      ...
      {c.creatorCard?.name && (
        <section className="px-6 py-8" style={{ background: "var(--m-lavender)" }}>
      ...
      {c.pressBar && (
        <section className="bg-white px-6 py-8 text-center">
          ...
          <div className="mt-4 flex flex-wrap items-center justify-center gap-x-8 gap-y-3 text-sm font-bold uppercase tracking-wide text-black/40">
            {BRAND.pressLogos.map((p) => (
              <span key={p} className="lp-serif normal-case italic">{p}</span>
            ))}
          </div>
            ...
            {c.reasons.map((r) => (
              <div key={r.n} className="flex gap-4">
                ...
                <div>
                  ...
                  {r.creatorQuote && (
                    <blockquote
                      className="mt-3 rounded-xl p-3 text-sm italic"
      ...
      {/* Before / after */}
      {c.beforeAfter && (
        <section className="px-6 py-10 text-center" style={{ background: "var(--m-beige)" }}>
      ...
      {c.mechanism && (
        <section className="bg-white px-6 py-10 text-center">
          ...
          <div className={`mx-auto mt-6 grid max-w-3xl gap-3 ${mobile ? "" : "grid-cols-2"}`}>
      ...
      {/* Clinical stats */}
      <section className="px-6 py-12 text-center text-white" style={{ background: "var(--m-text)" }}>
      ...
      {/* Offer */}
      <section className="px-6 py-12" style={{ background: "var(--m-pink)" }}>
      ...
      {/* Reviews */}
      <section className="px-6 py-10" style={{ background: "var(--m-beige)" }}>
      ...
      {/* Guarantee */}
      <section className="px-6 py-12 text-center" style={{ background: "var(--m-pale-pink)" }}>
      ...
      {/* Sticky ATC bar */}
      <div className="sticky bottom-0 flex items-center justify-between gap-3 border-t border-black/10 bg-white/95 px-4 py-3 backdrop-blur">
